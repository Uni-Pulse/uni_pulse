...
class EventNotifier extends StateNotifier<List<EventData>> {
  ...
  Future<String> addEvent(String eventName, String organisation, DateTime date,
      String ticketPrice, EventType eventType, String description) async {
    ...
    try {
      ...
      final events = querySnapshot.docs.map((doc) {
        ...
        return EventData(
          ...
          eventType: EventType.values.firstWhere((e) =>
              e.name ==
              data['eventType']
                  as String), // Adjust based on your EventType enum
          description: data['description']
              as String, // Assuming description is stored in Firestore
...
final eventsProvider = StateNotifierProvider<EventNotifier, List<EventData>>(
  (ref) => EventNotifier(),
);
  ...
  Future<void> updateUser({
    ...
    try {
      ...
      await firestore.collection('users').doc(userId).update({
        ...
        'email': email,
        'phoneNum':
            int.parse(phoneNum), // Assuming phoneNum is stored as an int
  ...
  Future<AccountData?> authenticate(String email, String password) async {
    try {
      ...
      if (userDoc.exists && userDoc.data() != null) {
        ...
        final orgDoc =
            await firestore.collection('organisations').doc(uid).get();
        if (orgDoc.exists && orgDoc.data() != null) {
          ...
          debugPrint(
              'No user or organisation details found in Firestore for UID: $uid');
          return null;
        }
      }
